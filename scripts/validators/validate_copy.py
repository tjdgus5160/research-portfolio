...
def main() -> int:
    ...
    # The old design was derived from a template, so the page had to say whose.
    # The 8-bit interface is original and there is nothing left to attribute, so
    # the line now states how the artwork was made instead. That claim is still
    # worth holding the page to: the pixel art is emitted by
    # scripts/gen_gb_assets.py, not produced by an image model.
    CREDIT = "ORIGINAL PIXEL ART, GENERATED BY CODE"
    hay = html.replace(CREDIT, "")
    checks.append(("attribution", "the provenance line is present",
                   CREDIT in html,
                   "" if CREDIT in html else "the page must say how its artwork was made"))
